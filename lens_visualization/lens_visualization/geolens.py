...
try:
    ...
    from .torchvision_compat import save_image
...
class GeoLens(Lens, GeoLensEval, GeoLensVis, GeoLensIO):
    ...
    def sample_grid_rays(
        ...
        This function is usually used for (1) PSF map, and (2) spot diagram calculation.
    ...
    def sample_radial_rays(
        ...
        This function is usually used for (1) PSF radial map calculation.
    ...
    def psf_huygens(self, points, ks=PSF_KS, wvln=DEFAULT_WAVE, spp=SPP_COHERENT, recenter=True):
        ...
        x_coords = torch.linspace(-psf_half_size + pixel_size/2, psf_half_size - pixel_size/2, ks, device=device)
        y_coords = torch.linspace(psf_half_size - pixel_size/2, -psf_half_size + pixel_size/2, ks, device=device)
        ...
        for batch_start in range(0, num_valid, batch_size):
            ...
            phase = torch.fmod((total_opl - opl_min) / wvln_mm, 1.0) * (2 * torch.pi)  # [ks, ks, batch]
    ...
    def psf_center(self, points, method="chief_ray"):
        ...
        if method == "chief_ray":
            ...
            if points[..., 2].min().abs() < 100:
                print("Point source is too close, pinhole model may be inaccurate for PSF center calculation.")
    ...
    def calc_focal_plane(self, wvln=DEFAULT_WAVE):
        ...
        if len(focus_z) > 0:
            ...
            raise ValueError("No valid rays found, focal plane in the image space cannot be computed.")
    ...
    def calc_fov(self):
        ...
        # 如果rfov是从JSON读取的，使用它；否则计算
        if hasattr(self, "_rfov_from_json") and self._rfov_from_json:
            # 使用JSON中提供的rfov值（已经在read_lens_json中设置）
            # 根据rfov计算其他视场角
            self.dfov = 2 * self.rfov  # 对角线视场角 = 2 * 半对角线视场角
            # vfov和hfov仍然根据sensor_size和foclen计算
            self.vfov = 2 * math.atan(self.sensor_size[0] / 2 / self.foclen)
            self.hfov = 2 * math.atan(self.sensor_size[1] / 2 / self.foclen)
        else:
            # 1. Perspective projection (effective FoV) - 计算所有视场角
            self.vfov = 2 * math.atan(self.sensor_size[0] / 2 / self.foclen)
            self.hfov = 2 * math.atan(self.sensor_size[1] / 2 / self.foclen)
    ...
    def calc_pupil(self):
        ...
        # Compute F-number
        # 如果fnum是从JSON读取的，使用它；否则根据入瞳半径计算
        if hasattr(self, "_fnum_from_json") and self._fnum_from_json:
            # 使用JSON中提供的fnum值（已经在read_lens_json中设置）
            pass  # self.fnum已经在read_lens_json中设置了
        else:
            # 根据入瞳半径计算fnum
    ...
    def read_lens_json(self, filename="./test.json"):
        ...
        with open(filename, "r") as f:
            ...
            for idx, surf_dict in enumerate(data["surfaces"]):
                ...
                if "material_display_name" in surf_dict:
                    s.material_display_name = surf_dict["material_display_name"]
        ...
        # 如果JSON中有fnum字段，读取它（用于显示和计算）
        if "fnum" in data:
            self.fnum = float(data["fnum"])
            self._fnum_from_json = True  # 标记fnum是从JSON读取的
        else:
            self._fnum_from_json = False  # 标记fnum需要计算
        
        # 如果JSON中有rfov字段，读取它（用于光线追迹时的视场角）
        # rfov应该是弧度值
        if "rfov" in data:
            self.rfov = float(data["rfov"])
            self.float_rfov = False  # 使用JSON中的值，不重新计算
            self._rfov_from_json = True  # 标记rfov是从JSON读取的
        else:
            self.float_rfov = False  # 如果没有，会在calc_fov中计算
            self._rfov_from_json = False  # 标记rfov需要计算
